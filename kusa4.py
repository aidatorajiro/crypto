...
# polynomial fitting (num of points is less than degree + 1)
...
# projection
def projection(mat, vec):
    mat = mat.copy()
    vec = vec.copy()
    mat_t = mat.t()
    return mat * dotinv(mat_t*mat) * mat_t * vec
...
class Mat(object):
    ...
    def map_dest(self, f):
        ...
        return self
    def t(self):
        return Mat([self.d[(i // self.n) + (i % self.n)*self.m] for i in range(self.n*self.m)],
            self.m, self.n)
    def map(self, f):
