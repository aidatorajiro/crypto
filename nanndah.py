# Diffie-Hellman key exchange using polynomial fitting over a finite field.
# Given k - 2 random points and 3 random numbers, (commutative) relationship between three numbers can be observed.
# Using the relationship R, we can construct a group such that R(x, y, z) <=> x + y + z = 0.
# cryptographic strength is not guaranteed, but fun :)
...
# given k points, return k coefficients.
def polyfit(points_x, points_y, k):
    lhs = vander(points_x, k)
    rhs = points_y
    solution = (dotinv(lhs)*rhs).getcol(0)
    return solution

# substitute x to a polynomial with coefficients coeffs
...
def modrand(p):
    return Mod(random.randint(0, p - 1), p)

# modulo of the finite field
p = 0x1630754518592437521810394623170439071787346163136715732951116994613647026908158243257902189

# degree of the polynomial + 1
k = 15

# (k - 2) predefined points
points_fixed_x = []
points_fixed_y = []

# 3 predefined x components
